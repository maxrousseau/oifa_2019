...
load('validationSetVgg16.mat','valDs','valImgs');
load vggnet
...
trainDs = augmentedImageDatastore([224 224],trainImgs,'DataAugmentation',imageAugmenter);
valDs = augmentedImageDatastore([224 224],valImgs,'DataAugmentation',imageAugmenter);

%pretrained VGG16 can be found from https://www.mathworks.com/help/deeplearning/ref/vgg16.html
net = vgg16;
trainingFeatures = activations(net,trainDs,'fc8','OutputAs','rows');

%% Baseline accuracy with clasical machine learning
classifier = fitcecoc(trainingFeatures,trainImgs.Labels);

valFeatures = activations(net,valDs,'fc8','OutputAs','rows');
predictedLabels = predict(classifier,valFeatures);

acc = nnz(predictedLabels == valImgs.Labels)/numel(predictedLabels);
%confusionchart(valImgs.Labels,predictedLabels);

%% Transfer Learning : Replace the fully connected layer of VGG16 net and the classification layer and train
ly = net.Layers;
...
options = trainingOptions('sgdm','MaxEpochs',50,'InitialLearnRate',0.00003,"Plots","training-progress",...
